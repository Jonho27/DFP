...
public class HandgunScriptLPFP : MonoBehaviour {
	...
	private void Start () {
		...
		if(grenades != null)
        {
			grenades.text = "x" + numberOfGrenades.ToString();
		}
	...
	private void Update () {
		...
		if (Input.GetButton("KnifeAttack") && !isInspecting) 
		{
        ...
        //Shooting 
        if (Input.GetButtonDown("Fire1") && !outOfAmmo && !isReloading && !isInspecting && !isRunning && !PauseMenu.isPaused) 
		{
			anim.Play ("Fire", 0, 0f);
		...
		//Reload 
		if (Input.GetButton("Reload") && !isReloading && !isInspecting && ammo != 0 && currentAmmo < gunMag) 
		{
			//Reload
			Reload ();
	...
	private IEnumerator AutoReload () {
        ...
        if (currentAmmo < gunMag)
        {
            if (ammo == (gunMag - currentAmmo))
            {
                currentAmmo = gunMag;
                ammo = 0;
            }

            else if (ammo > (gunMag - currentAmmo))
            {
                ammo -= gunMag - currentAmmo;
                currentAmmo = gunMag;
