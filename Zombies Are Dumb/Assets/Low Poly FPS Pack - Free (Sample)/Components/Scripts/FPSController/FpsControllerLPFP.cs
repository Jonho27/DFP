...
#pragma warning restore 649
        ...
        {
            if (other.transform.tag == "Target" && Input.GetButton("KnifeAttack"))
            {
                target = other;
                StartCoroutine(targetAbatido());
            }
        }

        

        IEnumerator targetAbatido()
        {
            yield return new WaitForSeconds(0.75f);
            target.GetComponentInChildren<TargetScript>().isHit = true;
