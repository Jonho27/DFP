...
public class GrenadeScript : MonoBehaviour {
	...
	private IEnumerator ExplosionTimer () 
	{
		...
		foreach (Collider hit in colliders) {
			...
			if (hit.tag == "Blue" || hit.tag == "Orange" || hit.tag == "Genetic" || hit.tag == "ZombiesNew" || hit.tag == "ZombieFlies" || hit.tag == "ProtectiveZombie" || hit.tag == "Bat")


            {
                hit.gameObject.GetComponent<Vida>().recibirDaño(70f);
            }

            if(hit.tag == "Player" || hit.tag == "Brother")
            {
                hit.gameObject.GetComponent<Vida>().recibirDaño(50f);
            }
