...
public class GrenadeScript1 : MonoBehaviour {
	...
	private IEnumerator ExplosionTimer () 
	{
        ...
        foreach (Collider hit in colliders)
        {
            ...
            if (hit.GetComponent<Collider>().tag == "Target"
                    && hit.GetComponentInChildren<TargetScript>().isHit == false)
            {
                //Animate the target 
                hit.GetComponentInChildren<Animation>().Play("target_down");
                //Toggle "isHit" on target object
                hit.GetComponentInChildren<TargetScript>().isHit = true;
            }
